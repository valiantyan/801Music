...
- [x] **AC2**: 点击歌曲时，自动将当前列表中的所有歌曲添加到播放队列
- [ ] **AC3**: 播放当前选中的歌曲（不是从队列第一首开始）
- [ ] **AC4**: 支持播放/暂停控制
- [ ] **AC5**: 支持上一首/下一首切换
- [ ] **AC6**: 显示当前播放歌曲的信息（标题、艺术家、封面）
- [x] **AC7**: 显示播放进度（当前位置和总时长）
- [x] **AC8**: 支持进度条拖拽跳转到指定位置
- [ ] **AC9**: 播放启动延迟 < 500ms（从点击到开始播放）
- [x] **AC10**: 播放过程中，列表中的当前播放歌曲有视觉标识
- [x] **AC11**: 播放队列支持循环播放（播放完最后一首后回到第一首）
...
- [x] **Task 6**: 实现 PlayerViewModel（队列管理）
...
- [x] **Task 9**: 实现 PlayerFragment（播放控制）
...
- [x] 2025-01-27 运行单元测试：`./gradlew :app:testDebugUnitTest`。
- [x] 2025-01-27 DoD 检查：任务清单与测试通过，文档同步完成，清理无临时代码；UI 截图/组件测试待补充。
