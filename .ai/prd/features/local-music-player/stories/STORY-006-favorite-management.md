...
- [ ] **AC12**: 收藏功能不影响播放性能
- [ ] **AC13**: 配置变更场景支持：
  - 配置变更后，收藏状态 UI 正确恢复（通过数据库查询）
  - 屏幕旋转、主题切换、分屏模式下，收藏按钮状态正确显示
  - 配置变更时正在进行的收藏操作不应丢失（通过 ViewModel 保证）
  - 收藏按钮图标在主题切换后正确适配
- [ ] **UI/UX**: 
...
- [ ] **Integration Tests**: 
  - 收藏功能端到端测试通过
  - 配置变更测试：收藏操作后旋转屏幕，验证收藏状态恢复
  - 配置变更测试：切换主题，验证收藏按钮图标适配
...
4. **状态同步**
   ...
   - **配置变更处理**：
     - 收藏数据通过 Room 数据库持久化，配置变更后从数据库恢复
     - 收藏状态通过 ViewModel StateFlow 自动保存和恢复
     - 配置变更后重新查询收藏状态，确保 UI 正确显示
     - 主题切换时收藏按钮图标正确适配
...
- [ ] **Task 12**: 集成测试和性能优化
    ...
    - *Test Case*: 配置变更测试 - 收藏操作后各种配置变更场景（旋转、主题、分屏）
